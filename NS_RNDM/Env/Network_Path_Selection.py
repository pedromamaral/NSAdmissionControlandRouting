...
import random
import Env.NetworkX_API_Topo as netX
...
from Env.Containernet_API_Topo import ContainernetAPI
from Env.Parameters_NPS import BASE_STATIONS, COMPUTING_STATIONS, CONNECTIONS_OFFSET,\
    ...
    def step(self, action):
        ...
        if self.state[0]:
            ...
            if action < 6:
                ...
                print("Path:",action)
                self.create_slice(*slice_connections_from_array(self.state[4:CONNECTIONS_OFFSET]),action)
                ...
                print(f"REJECT")
                rejected_action = action - 6
                print ("Path: ", rejected_action)
                reward = 0
        ...
        if self.requests < MAX_REQUESTS:
            ...
            for evaluator in self.evaluators:
                if evaluator.is_alive():  # might get stuck if a second evaluator finishes before this one
                    ...
                    # print(self.state)
                    return self.state, reward, done, accepted_eslices, accepted_islices, total_requests, {}
        ...
        print('Duration: ', self.state[1])
        print('Bw: ', self.state[2])
        print('Cost: ', self.state[3])
        print('Reward: ', reward)
        print('\n\nEND OF STEP ENV\n\n')
        return self.state, reward, done, accepted_eslices, accepted_islices, total_requests, {}
...
#***************************************************************************************************************************************
    ...
    def create_slice(self, clients, servers, action):
        ...
        for (client, server) in zip(clients, servers):
            ...
            #file.write("\nEste deu erro: \n")
            #with open("CR2.txt", "w") as file2:
                #file2.write("VECTOR IN CREATE:")
                #for item in self.active_connections:
                    #file2.write("%s\n" % item)
                    #file.close()
                        
                #file2.write(" CREATE END --------\n")
                #file2.write(f'{client}_{server}_{port}')
                #file2.write("\n")

            self.debug_vector = [f'{client}_{server}_{port}']
            
            #print(create: Para ficheiro client server port)
            #iperf
    ...
    def slice_evaluator(self, clients, servers, ports, slice_type, duration, bw, price, action) :
        ...
        for (client, server, port) in zip(clients, servers, ports):
            debug_vector2 = [f'{clients}_{servers}_{ports}']
        ...
        if data:
            if slice_type == 1:
                reward, failed_e = evaluate_elastic_slice(bw, price, data) 
                #cnt_e += failed_e
                print('SLICE TYPE 1\n\n')
            else:
                reward, failed_i = evaluate_inelastic_slice(bw, price, data)
                #cnt_i += failed_i                
                print('SLICE TYPE 2\n\n')
        ...
        '''with open('Failed_elastic.txt', 'a') as file:
            send_to_file1 = str(cnt_e) + "\n"
            file.write(send_to_file1)

        with open('Failed_inelastic.txt', 'a') as file2:
            send_to_file2 = str(cnt_i) + "\n"
            file2.write(send_to_file2)'''
...
def evaluate_elastic_slice(bw, full_price, data):
    ...
    if total_average >= bw - bw * .1:
        print("\n\n\t\tFinished elastic slice {total_average} >= {bw}\n\n")
        aux=0
        return 0.0, aux
    print("Failed elastic slice {total_average} < {bw}")
    return - full_price / 2, aux
...
def evaluate_inelastic_slice(bw, price, data):
    ...
    if worst >= bw - bw * .1:
        print("\n\n\t\tFinished inelastic slice {worst} >= {bw}\n\n")
        aux=0
        return 0.0, aux
    print("Failed inelastic slice {worst} < {bw}")
    return - price, aux
